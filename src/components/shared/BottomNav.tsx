...
export function BottomNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 z-50 w-full border-t border-slate-100 bg-white shadow-[0_-4px_20px_rgba(0,0,0,0.02)] h-[80px] pb-safe rounded-t-3xl flex justify-around items-center px-2">
      ...
      <Link href="/debts" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${pathname === '/debts' ? 'text-[#6F4A12]' : 'text-slate-400'}`}>
        <div className={`p-2 rounded-2xl ${pathname === '/debts' ? 'bg-[#F7E7C7]' : 'bg-transparent'}`}>
          <CircleDollarSign className="h-7 w-7 stroke-[2.5]" />
        </div>
        <span className="text-xs font-bold uppercase tracking-tighter">หนี้ลูกค้า</span>
      </Link>
    </div>
  )
}
